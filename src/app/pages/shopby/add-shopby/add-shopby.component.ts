...
@Component({
	selector: 'app-add-shopby',
	templateUrl: './add-shopby.component.html',
	styleUrls: ['./add-shopby.component.scss']
})
export class AddShopbyComponent {
	...
	constructor(
		...
		this.addshopbyForm = this.formBuilder.group({
	...
	public hasError = (controlName: string, errorName: string) => {
		return this.addshopbyForm.controls[controlName].hasError(errorName);
	};
	...
	patchingdata(id: any) {
		let obj = { id: id };
		this.shopbyService.getShopbyWithId(obj).subscribe(
			(response) => {
				if (response.code == 200) {
					let data = response?.result;
					this.shopbyData = response.result;
					this.shopbyImage = data?.image;
					this.mediaData = data?.media_data[0];
					this.ImageName = data?.mobile_image;
					this.addshopbyForm.patchValue({
						name: data?.name,
						status: data?.status,
	...
	onSubmit() {
		this.submitted = true;
		let obj = this.addshopbyForm.value;
		let id = this.id;
		obj['token'] = this.token;
		obj['mobile_image'] = this.ImageName;
		if (this.addshopbyForm.invalid) {
			return;
		}
		if (this.mediaData) {
		...
		if (!this.isEdit) {
			this.shopbyService.addShopby(obj).subscribe(
				(response) => {
					if (response.code == 200) {
						...
						setTimeout(() => {
							this.router.navigate(['/shopby/view']);
						}, 2000);
					}
					else {
						this.throw_msg = response.message
			...
			this.shopbyService.editShopbydata(obj, id).subscribe(
				(response) => {
					if (response.code == 200) {
						this.throw_msg = response.message
						this.msg_success = true;
	...
	onUploadOutput(output: UploadOutput, typeofImage): void {
		if (output.type === 'allAddedToQueue') {
			const event: UploadInput = {
				type: 'uploadAll',
				url: environment.baseUrl + '/api/shopby/addimage',
				method: 'POST',
				data: {},
			};
				...
				this.mediaService.editMediadata(obj, id).subscribe(
					(response) => {
						if (response.code == 200) {
							...
							this.mediaData.src = response.result.src;
							if (this.shopbyData.media_data && this.shopbyData.media_data.length > 0) {
								this.patchingdata(this.id);
							}
							this.modalService.dismissAll();
						} else {
