...
@Component({
  selector: 'app-add-cartype',
  templateUrl: './add-cartype.component.html',
  styleUrls: ['./add-cartype.component.scss']
})
export class AddCartypeComponent {
  ...
  @ViewChild('uploader', { read: ElementRef }) fileInput: ElementRef;
	...
	public hasError = (controlName: string, errorName: string) => {
		return this.addcartypeForm.controls[controlName].hasError(errorName);
	};
	...
	patchingdata(id: any) {
		let obj = { id: id };
		this.cartypeService.getCartypeWithId(obj).subscribe(
			(response) => {
				if (response.code == 200 && response?.result.length > 0) {
					...
					this.addcartypeForm.patchValue({
						name: data?.name,
						title: data?.title,
						status: data?.status,
	...
	onSubmit() {
		this.submitted = true;
		let obj = this.addcartypeForm.value;
		let id = this.id;
		obj['token'] = this.token;
		obj['image'] = this.images;
		obj['gallery_image'] = this.gallery_images;
		let categoriesData: any = [];
		if (this.addcartypeForm.value.categories && this.addcartypeForm.value.categories.length > 0) {
			this.addcartypeForm.value.categories.forEach((category) => {
				let obj = {
		...
		if (!this.isEdit) {
			this.cartypeService.addCartype(obj).subscribe(
				(response) => {
			...
			this.cartypeService.editCartypedata(obj, id).subscribe(
				(response) => {
					if (response.code == 200) {
						this.throw_msg = response.message
						this.msg_success = true;
	...
	onUploadOutputMedia(output: UploadOutput, type): void {
		if (output.type === 'allAddedToQueue') {
			const event: UploadInput = {
				type: 'uploadAll',
				url: environment.baseUrl + '/api/cartype/addimage',
				method: 'POST',
				data: {},
			};
	...
	onUploadVideo(output: UploadOutput, type): void {
		if (output.type === 'allAddedToQueue') {
			const event: UploadInput = {
				type: 'uploadAll',
				url: environment.baseUrl + '/api/cartype/addVideo',
				method: 'POST',
				data: {},
			};
			this.uploadInput.emit(event);
						...
						if (response.code == 200) {
							...
							if (type == 'gallery') {
								if (this.galleryData && this.galleryData.length > 0) {
									...
									if (this.cartypeData.gallery_image && this.cartypeData.gallery_image.length > 0) {
										this.patchingdata(this.id);
									}
									this.modalService.dismissAll();
								}
								...
								if (this.mediaData && this.mediaData.length > 0) {
									...
									if (this.cartypeData.media_data && this.cartypeData.media_data.length > 0) {
										this.patchingdata(this.id);
									}
									this.modalService.dismissAll();
								}
	...
	deleteMediaFile() {
		if (this.isUploaded && this.deletedMediaFile && this.deletedMediaFile.length > 0) {
			...
			this.cartypeService.deletefile(obj).subscribe(
				(response) => {
					if (response.code == 200) {
	...
	deleteMediaMultiFiles() {
		if (this.deletedMediaFiles) {
			...
			this.cartypeService.deleteMediaData(obj).subscribe(
				(response) => {
					if (response.code == 200) {
						this.isUploaded = false;
						if (this.galleryFile) {
