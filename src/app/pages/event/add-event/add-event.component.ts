...
export class AddEventComponent {
	...
	patchingdata(id: any) {
		let obj = { _id: id };
		this.eventService.getEventWithId(obj).subscribe(
			(response) => {
	...
	onSubmit() {
		this.submitted = true;
		let obj = this.addeventForm.value;
		let id = this.id;
		...
		if (!this.isEdit) {
			this.eventService.addEvent(obj).subscribe(
				(response) => {
					if (response.code == 200) {
			...
			this.eventService.editEventdata(obj, id).subscribe(
				(response) => {
					if (response.code == 200) {
						this.throw_msg = response.message
						this.msg_success = true;
	...
	onUploadOutput(output: UploadOutput, typeofImage): void {
		if (output.type === 'allAddedToQueue') {
			const event: UploadInput = {
				type: 'uploadAll',
				url: environment.baseUrl + '/api/event/addimage',
				method: 'POST',
				data: {},
			};
				...
				this.mediaService.editMediadata(obj, id).subscribe(
					(response) => {
						if (response.code == 200) {
							...
							this.mediaData.src = response.result.src;
							if (this.eventData.media_data && this.eventData.media_data.length > 0) {
								this.patchingdata(this.id);
							}
							this.modalService.dismissAll();
						} else {
	...
	getCustomerGroupsData() {
		let obj = {};
		this.authorService.getAllAuthors(obj).subscribe(
			(response) => {
				if (response.code == 200) {
