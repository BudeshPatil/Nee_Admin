...
@ViewChild('uploader', { read: ElementRef }) fileInput: ElementRef;
  ...
  public hasError = (controlName: string, errorName: string) => {
    return this.addmodelForm.controls[controlName].hasError(errorName);
  };
  ...
  patchingdata(id: any) {
    let obj = { id: id };
    this.modelService.getModelWithId(obj).subscribe(
      (response) => {
        if (response.code == 200 && response?.result.length > 0) {
          let data = response?.result[0];
          this.modelData = response?.result[0];
          if (data?.media_data) {
  ...
  onSubmit() {
    this.submitted = true;
    let obj = this.addmodelForm.value;
    let id = this.id;
    ...
    if (!this.isEdit) {
      this.modelService.addModel(obj).subscribe(
        (response) => {
          if (response.code == 200) {
      ...
      this.modelService.editModeldata(obj, id).subscribe(
        (response) => {
          if (response.code == 200) {
            this.throw_msg = response.message
            this.msg_success = true;
  ...
  onUploadOutputMedia(output: UploadOutput, type): void {
    if (output.type === 'allAddedToQueue') {
      const event: UploadInput = {
        type: 'uploadAll',
        url: environment.baseUrl + '/api/model/addimage',
        method: 'POST',
        data: {},
      };
  ...
  onUploadVideo(output: UploadOutput, type): void {
    if (output.type === 'allAddedToQueue') {
      const event: UploadInput = {
        type: 'uploadAll',
        url: environment.baseUrl + '/api/model/addVideo',
        method: 'POST',
        data: {},
      };
      this.uploadInput.emit(event);
            ...
            if (response.code == 200) {
              ...
              if (type == 'gallery') {
                if (this.galleryData && this.galleryData.length > 0) {
                  ...
                  if (this.modelData.gallery_image && this.modelData.gallery_image.length > 0) {
                    this.patchingdata(this.id);
                  }
                  this.modalService.dismissAll();
                }
                ...
                if (this.mediaData && this.mediaData.length > 0) {
                  ...
                  if (this.modelData.media_data && this.modelData.media_data.length > 0) {
                    this.patchingdata(this.id);
                  }
                  this.modalService.dismissAll();
                }
  ...
  deleteMediaFile() {
    if (this.isUploaded && this.deletedMediaFile && this.deletedMediaFile.length > 0) {
      ...
      this.modelService.deletefile(obj).subscribe(
        (response) => {
          if (response.code == 200) {
  ...
  deleteMediaMultiFiles() {
    if (this.deletedMediaFiles) {
      ...
      this.modelService.deleteMediaData(obj).subscribe(
        (response) => {
          if (response.code == 200) {
            this.isUploaded = false;
            if (this.galleryFile) {
