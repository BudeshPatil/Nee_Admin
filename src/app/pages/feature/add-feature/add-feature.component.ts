...
export class AddFeatureComponent {
  ...
  public hasError = (controlName: string, errorName: string) => {
    return this.addfeatureForm.controls[controlName].hasError(errorName);
  };
  ...
  get f() {
    return this.addfeatureForm.controls;
  }

  patchingdata(id: any) {
    let obj = { id: id };
    this.featureService.getFeatureWithId(obj).subscribe(
      (response) => {
        if (response.code == 200 && response?.result.length > 0) {
          let data = response?.result[0];
          this.featureData = response?.result[0];
          if (data?.media_data) {
  ...
  onSubmit() {
    this.submitted = true;
    let obj = this.addfeatureForm.value;
    let id = this.id;
    obj['token'] = this.token;
    obj['image'] = this.images;
    obj['gallery_image'] = this.gallery_images;
    let categoriesData: any = [];
    if (this.addfeatureForm.value.categories && this.addfeatureForm.value.categories.length > 0) {
      this.addfeatureForm.value.categories.forEach((category) => {
        let obj = {
    ...
    if (!this.isEdit) {
      this.featureService.addFeature(obj).subscribe(
        (response) => {
      ...
      this.featureService.editFeaturedata(obj, id).subscribe(
        (response) => {
          if (response.code == 200) {
            this.throw_msg = response.message
            this.msg_success = true;
  ...
  onUploadOutputMedia(output: UploadOutput, type): void {
    if (output.type === 'allAddedToQueue') {
      const event: UploadInput = {
        type: 'uploadAll',
        url: environment.baseUrl + '/api/feature/addimage',
        method: 'POST',
        data: {},
      };
  ...
  onUploadVideo(output: UploadOutput, type): void {
    if (output.type === 'allAddedToQueue') {
      const event: UploadInput = {
        type: 'uploadAll',
        url: environment.baseUrl + '/api/feature/addVideo',
        method: 'POST',
        data: {},
      };
      this.uploadInput.emit(event);
            ...
            if (response.code == 200) {
              ...
              if (type == 'gallery') {
                if (this.galleryData && this.galleryData.length > 0) {
                  ...
                  if (this.featureData.gallery_image && this.featureData.gallery_image.length > 0) {
                    this.patchingdata(this.id);
                  }
                  this.modalService.dismissAll();
                }
                ...
                if (this.mediaData && this.mediaData.length > 0) {
                  ...
                  if (this.featureData.media_data && this.featureData.media_data.length > 0) {
                    this.patchingdata(this.id);
                  }
                  this.modalService.dismissAll();
                }
  ...
  deleteMediaFile() {
    if (this.isUploaded && this.deletedMediaFile && this.deletedMediaFile.length > 0) {
      ...
      this.featureService.deletefile(obj).subscribe(
        (response) => {
          if (response.code == 200) {
  ...
  deleteMediaMultiFiles() {
    if (this.deletedMediaFiles) {
      ...
      this.featureService.deleteMediaData(obj).subscribe(
        (response) => {
          if (response.code == 200) {
            this.isUploaded = false;
            if (this.galleryFile) {
