...
@Component({
  selector: 'app-add-boudytype',
  templateUrl: './add-boudytype.component.html',
  styleUrls: ['./add-boudytype.component.scss']
})
export class AddBoudytypeComponent {
  @ViewChild('uploader', { read: ElementRef }) fileInput: ElementRef;
	...
	constructor(
		...
		this.addbodytypeForm = this.formBuilder.group({
	...
	public hasError = (controlName: string, errorName: string) => {
		return this.addbodytypeForm.controls[controlName].hasError(errorName);
	};
	...
	get f() {
		return this.addbodytypeForm.controls;
	}

	patchingdata(id: any) {
		let obj = { id: id };
		this.bodytypeService.getBodytypeWithId(obj).subscribe(
			(response) => {
				if (response.code == 200 && response?.result.length > 0) {
					let data = response?.result[0];
					this.bodytypeData = response?.result[0];
					if (data?.media_data) {
					...
					this.addbodytypeForm.patchValue({
						name: data?.name,
						title: data?.title,
						status: data?.status,
						type: data?.type,
	...
	onSubmit() {
		this.submitted = true;
		let obj = this.addbodytypeForm.value;
		let id = this.id;
		obj['token'] = this.token;
		obj['image'] = this.images;
		obj['gallery_image'] = this.gallery_images;
		let categoriesData: any = [];
		if (this.addbodytypeForm.value.categories && this.addbodytypeForm.value.categories.length > 0) {
			this.addbodytypeForm.value.categories.forEach((category) => {
				let obj = {
		...
		if (this.addbodytypeForm.invalid) {
			return;
		}
		if (!this.isEdit) {
			this.bodytypeService.addBodytype(obj).subscribe(
				(response) => {
					if (response.code == 200) {
						...
						setTimeout(() => {
							this.router.navigate(['/bodytype/view']);
						}, 2000);
					}
					else {
						this.throw_msg = response.message
			...
			this.bodytypeService.editBodytypedata(obj, id).subscribe(
				(response) => {
					if (response.code == 200) {
						this.throw_msg = response.message
						this.msg_success = true;
	...
	onUploadOutputMedia(output: UploadOutput, type): void {
		if (output.type === 'allAddedToQueue') {
			const event: UploadInput = {
				type: 'uploadAll',
				url: environment.baseUrl + '/api/bodytype/addimage',
				method: 'POST',
				data: {},
			};
	...
	onUploadVideo(output: UploadOutput, type): void {
		if (output.type === 'allAddedToQueue') {
			const event: UploadInput = {
				type: 'uploadAll',
				url: environment.baseUrl + '/api/bodytype/addVideo',
				method: 'POST',
				data: {},
			};
			this.uploadInput.emit(event);
						...
						if (response.code == 200) {
							...
							if (type == 'gallery') {
								if (this.galleryData && this.galleryData.length > 0) {
									...
									if (this.bodytypeData.gallery_image && this.bodytypeData.gallery_image.length > 0) {
										this.patchingdata(this.id);
									}
									this.modalService.dismissAll();
								}
								...
								if (this.mediaData && this.mediaData.length > 0) {
									...
									if (this.bodytypeData.media_data && this.bodytypeData.media_data.length > 0) {
										this.patchingdata(this.id);
									}
									this.modalService.dismissAll();
								}
	...
	deleteMediaFile() {
		if (this.isUploaded && this.deletedMediaFile && this.deletedMediaFile.length > 0) {
			...
			this.bodytypeService.deletefile(obj).subscribe(
				(response) => {
					if (response.code == 200) {
	...
	deleteMediaMultiFiles() {
		if (this.deletedMediaFiles) {
			...
			this.bodytypeService.deleteMediaData(obj).subscribe(
				(response) => {
					if (response.code == 200) {
						this.isUploaded = false;
						if (this.galleryFile) {
