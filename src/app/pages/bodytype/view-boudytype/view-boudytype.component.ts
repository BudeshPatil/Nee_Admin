...
@Component({
  selector: 'app-view-boudytype',
  templateUrl: './view-boudytype.component.html',
  styleUrls: ['./view-boudytype.component.scss']
})
export class ViewBoudytypeComponent {
  ...
  selectedCustomergroup : any;
	...
	get_BodytypeData() {
		...
		this.bodytypeService.getBodytypeDetails(obj).subscribe(
			(response) => {
	...
	deleteBodytype()
  {
    if (this.selectedBodytype) {
			var mylist = { id: this.selectedBodytype._id };
			mylist['token'] = this.token;
      this.bodytypeService.deletebodytype(mylist).subscribe(
        (response)=> {
          if (response.code == 200)
          {
						if(this.selectedBodytype && this.selectedBodytype.image){
							this.selectedBodytype.image.forEach(image => {
								if(image && image.src){
						...
						if(this.selectedBodytype && this.selectedBodytype.gallery_image){
							this.selectedBodytype.gallery_image.forEach(image => {
								if(image && image.src){
									this.deletedMediaFiles.push(image.src);
									this.deletMediaFilesData.push(image);
								}
  ...
  closeModal(){
	...
	deleteMediaData() {
		if (this.deletMediaFilesData && this.deletMediaFilesData.length > 0) {
			...
			this.bodytypeService.deleteMediaData(mylist).subscribe(
				(response) => {
					if (response.code == 200) {
						this.modalService.dismissAll();
					}
