...
@Component({
  selector: 'app-view-yatchs',
  templateUrl: './view-yatchs.component.html',
  styleUrls: ['./view-yatchs.component.scss']
})
export class ViewYatchsComponent {
  ...
  msg_danger: boolean = false;
    ...
    deleteVehicle() {
      if (this.selectedVehicle) {
        var mylist = { id: this.selectedVehicle._id, home_vehicle: true };
        this.vehicleService.deletedisplayonehomevehicle(mylist).subscribe(
          (response) => {
            if (response.code == 200) {
              this.get_vehicleData();
              this.router.navigate(['/displayonhomeyatchs/view']);
