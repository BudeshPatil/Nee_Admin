...
@Component({
	selector: 'app-about-banner-view',
	templateUrl: './about-banner-view.component.html',
	styleUrls: ['./about-banner-view.component.scss']
})
export class AboutBannerViewComponent implements OnInit {
			...
			(response) => {
				if (response.code == 200) {
					if (response.result != null && response.result != '') {
						// Filter only 'about' banners and keep as array
						this.BannerData = response.result.filter(banner => banner.name === 'about');
						this.totalRecord = this.BannerData.length; // Update count for filtered data
					} else {
	...
	deleteBanner(): void {
		if (!this.selectedBanner || !this.selectedBanner._id) return;

		const payload = { id: this.selectedBanner._id };

		this.bannerservice.deletebanner(payload).subscribe({
			next: (response) => {
				if (response?.code === 200) {
					// Remove deleted banner from local array
					this.BannerData = this.BannerData.filter(
						banner => banner._id !== this.selectedBanner._id
					);

					this.deleteMediaData();
					this.modalService.dismissAll();

					this.toastr.successToastr('Banner deleted successfully!', 'Success');

				}
			},
			error: (error) => {
				console.error('Delete banner error:', error);
				this.toastr.errorToastr('Banner deletion failed.', 'Error');
			}
		});
	}
